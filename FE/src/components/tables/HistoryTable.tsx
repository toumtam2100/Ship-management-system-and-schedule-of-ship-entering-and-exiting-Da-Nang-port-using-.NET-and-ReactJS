...
const HistoryTable: React.FC = () => {
  ...
  useEffect(() => {
    if (arrival.length > 0 || departure.length > 0) {
      ...
      const combinedArray = arrival
        .filter(arrival => arrival.isStart === true && (userRole === 'PortAuthority' || arrival?.ship?.ownerId === Number(userId) || arrival?.captain?.id === Number(userId)))
        .map(arrival => ({
          id: arrival.arrivalId,
          shipName: arrival.ship.name,
          actualTime: arrival.arrivalTime,
        ...
        .concat(
          departure
            .filter(departure => departure.isStart === true && (userRole === 'PortAuthority' || departure?.ship?.ownerId === Number(userId) || departure?.captainId === Number(userId)))
            .map(departure => ({
              id: departure.departureId,
              shipName: departure.ship.name,
              actualTime: departure.departureTime,
  ...
  return (
    <>
      <Title style={titleStyle}>{`Lịch sử ra vào cảng`}</Title>
