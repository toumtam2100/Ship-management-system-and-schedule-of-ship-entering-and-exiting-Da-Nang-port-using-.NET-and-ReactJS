...
import { CreateBorderguardData, createBorderguard } from '@app/api/borderGuard.api';
const { Title } = Typography;

const BorderGuardForm: React.FC<{ onFormChange: (values: any) => void }> = ({ onFormChange }) => {
  ...
  //   message.success('Đăng ký thành công biên phòng');
  ...
  const handleFormFinish = async (values: any) => {
    try {
      ...
      // Assuming you have already set the provinceName, districtName, communeName, and streetValue states
      const fullAddress = `${provinceName}, ${districtName}, ${communeName}, ${streetValue}`;
      values.address = fullAddress;

      // Create the border guard data object to pass to the createBorderguard function
      const borderguardData: CreateBorderguardData = {
      ...
      // Call createBorderguard function to send the data to the API
      //const response = await createBorderguard(borderguardData);
      //console.log('Border guard created successfully:', response);

      if (selectedFile) {
        const uploadResult = await handleFinish(selectedFile, imageName);
        if (uploadResult) {
          borderguardData.avatar = uploadResult.Key;
          await createBorderguard(borderguardData);
          message.success('Đăng ký thành công cảng vụ');
          navigate('/borderguard-management');
        }
      } else {
...
export default BorderGuardForm;
