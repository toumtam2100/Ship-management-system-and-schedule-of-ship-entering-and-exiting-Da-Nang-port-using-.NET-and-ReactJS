...
const CaptainForm: React.FC<{ onFormChange: (values: any) => void }> = ({ onFormChange }) => {
  ...
  const successMessage = () => {
    message.success('Đăng ký thành công thuyền trưởng');
  ...
  const handleFormFinish = async (values: any) => {
    try {
      ...
      if (selectedFile) {
        await handleFinish(selectedFile, values);
      } else {
        console.error('selectedFile is undefined');
      }
    } catch (error) {
