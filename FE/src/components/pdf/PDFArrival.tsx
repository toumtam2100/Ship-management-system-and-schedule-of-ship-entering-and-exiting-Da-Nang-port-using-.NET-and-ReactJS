...
import { uploadFileToS3 } from './PDFArrival.utils';
...
interface SubmitRegistry {
  ...
  grossTonnage: string;
  arrivalTime: string; // Ensure the date is formatted as a string
  // Add other fields as needed
}

export const generatePDFArrival = async (submitRegistry: SubmitRegistry) => {
  const doc = new jsPDF({
  ...
  const arrivalDate = new Date(submitRegistry.arrivalTime);
  const arrivalYear = arrivalDate.getFullYear();
  const arrivalMonth = arrivalDate.getMonth() + 1; // JavaScript months are 0-indexed
  ...
  doc.text('GIẤY PHÉP VÀO CẢNG', pageCenterX, licenseTextYPosition, { align: 'center' });
  ...
  const textLines = [
    ...
    `Thời gian vào cảng: dự kiến ngày ${arrivalDay} tháng ${arrivalMonth} năm ${arrivalYear}`, // Format the date
  ...
  //const compressedShipName = clearShipName.replace(/\s/g, '');
